...
function lengthOfLongestSubstring(s) {
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        if (charMap.has(char)) {
            ...
            // console.log(`Next window: char: ${char} currentPos: ${i} last seen: ${lastSeen} start: ${start}`, currentLongest, i - start)
            start = start > lastSeen ? start : lastSeen + 1;
            currentLongest = currentLength > currentLongest ? currentLength : currentLongest;
        }
        else {
            charMap.set(char, i);
            // console.log("Set: ", char, i)
        }
    }
    let lastWinow = len - start;
    // console.log('Last winwod,', s.length, start, lastWinow)
...
testLongestSubstr();
//# sourceMappingURL=LongestNonRepeating.js.map